...
const addDURL = spacetimedb.reducer({
	creator: t.string(),
	url: t.string()
}, (ctx, { creator, url }) => {
	ctx.db.durl.insert({
...
const removeDURL = spacetimedb.reducer({
	id: t.u64().optional(),
	...
	if (url !== void 0 && url !== "") {
		for (const row of ctx.db.durl.iter()) if (row.url === url) {
